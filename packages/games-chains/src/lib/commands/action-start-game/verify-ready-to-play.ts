...
import {
  gameCommandMap,
  type NextCommandMap,
} from '../../utils/context-next-map.js';

export const verifyReadyToPlay = CommandBuilder.build(
  (context: Context<GameContextKeys | string>) => {
    if (
      ...
      if (readyToPlay) {
        ...
        const gameName =
          (context.get('req-params-id') as string) ??
          (req.params['id'] as string) ??
          'Chutes-&-Ladders';

        const nextGameCommandMap = gameCommandMap.get(
          gameName,
        ) as NextCommandMap;

        const nextCommand = nextGameCommandMap.get(

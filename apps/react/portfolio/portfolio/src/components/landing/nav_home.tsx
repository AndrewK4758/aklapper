...
interface NavToHomeProps extends Omit<BoxProps, 'component' | 'flex'> {
  isVisable: boolean;
  motionOffset: number;
...
export default function NavToHome({ isVisable, motionOffset, onHandleClickEnter, ...props }: NavToHomeProps) {
  const enterIconStyle: SxProps<Theme> = {
    ...
    transform: `translate(-150px, 135px) ${isVisable ? 'scale(2.25)' : 'scale(0.3)'}`,

    transition: 'offset-distance 1s ease-out, opacity 0.7s ease-in, transform 1.2s ease-out',
  };
  return (
    <Box {...props} component={'div'} flex={'0 1 auto'}>
